					...
					new TypeMapping(
						...
						new MetaType() {
							...
							public List<MetaType> getInterfaces() {
								...
								final var targets = interfaces
									.stream()
									.map(parserConfig.getTypeMapper()::getMappingForSource)
	...
	private SourceFile mirrorDto(Class<?> clazz) throws MirroringFailedException {
		...
		try {
			...
			for (final var field : fieldsToMirror) {
				...
				if (field.isAnnotationPresent(MirrorProperties.class)) {
					optionalField = field.getAnnotation(MirrorProperties.class).optional();
				}

				if (field.isAnnotationPresent(MirrorGeneric.class)) {
					final var annotation = field.getAnnotation(MirrorGeneric.class);
					parser = annotation.parser().getDeclaredConstructor().newInstance();
					metaField = parser.parse(field, annotation.types(), this.parserConfig.getTypeMapper(), optionalField);
				} else if (field.isAnnotationPresent(MirrorList.class)) {
					final var annotation = field.getAnnotation(MirrorList.class);
					parser = new NestedListFieldParser();
					metaField = parser.parse(field, annotation.types(), this.parserConfig.getTypeMapper(), optionalField);
				} else if (field.isAnnotationPresent(MirrorMap.class)) {
					final var annotation = field.getAnnotation(MirrorMap.class);
					parser = new MapFieldParser();
					metaField = parser.parse(field, annotation.types(), this.parserConfig.getTypeMapper(), optionalField);
				} else {
					parser = new DefaultFieldParser();
					metaField = parser.parse(field, new Class<?>[] {}, this.parserConfig.getTypeMapper(), optionalField); // TODO this is not optimal
				}
